...
def run_tmux_commands(session_name, commands, top_pane=None):
    ...
    try:
        ...
        # Last pane is a blank shell with sando_ws sourced
        ...
        subprocess.run(
            ["tmux", "send-keys", "-t", blank_pane,
             "source ~/code/sando_ws/install/setup.bash", "C-m"],
            check=True,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument('--planner', type=str, default="sando",
                        help="Planner to use (dynus, mighty, or sando)")
    ...
    commands = [
        ...
        ("SANDO", (
            f"source ~/code/sando_ws/install/setup.bash && "
            f"source ~/code/decomp_ws/install/setup.bash && "
            f"sleep 10 && "
            f"ros2 launch sando onboard_sando.launch.py "
            f"x:=0.0 y:=0.0 z:=0.0 yaw:=0 namespace:={veh} "
            f"use_obstacle_tracker:=false use_ground_robot:=false "
            f"use_hardware:=true use_onboard_localization:={'true' if use_onboard_loc else 'false'} "
            f"depth_camera_name:=d455"
    ...
    commands += [
        ...
        ("BAG RECORD", (
            ...
            f"mkdir -p ~/data/sando && "
            f"python3 ~/code/sando_ws/src/sando/scripts/bag_record.py "
            f"--bag_name $BAG_NAME --bag_path ~/data/sando --hardware --agents {veh}"
        )),
        ...
        ("ACL MAP", (
            ...
            f"ros2 launch global_mapper_ros global_mapper_node.launch.py namespace:={veh} depth_pointcloud_topic:=dlio/odom_node/pointcloud/deskewed use_tracker:=true publish_init_tf:=false"
        )),
        ...
        ("GOAL MONITOR", (
            f"source ~/code/sando_ws/install/setup.bash && "
            f"ros2 launch sando goal_monitor.launch.py"
        ), False),
